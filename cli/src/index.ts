...
program.name('dbforge').description('AI-native Postgres provisioning').version('0.1.0')
...
auth
  ...
  .requiredOption('--url <url>', 'dbforge API URL')
...
projects
  ...
  .action(async (id?: string) => {
    ...
    try {
      ...
      console.log(chalk.dim('Saved to ~/.dbforge/projects.json'))
    } catch (err: any) {
...
projects
  ...
  .action(() => {
    ...
    if (!entries.length) {
      console.log('No projects. Run: dbforge projects create')
      return
    }
  ...
  .action(async (projectId: string, opts) => {
    try {
      ...
      const lines: string[] = [
        ...
        `// Do not edit manually. Re-run: dbforge gen types ${projectId}`,
        '',
      ]
